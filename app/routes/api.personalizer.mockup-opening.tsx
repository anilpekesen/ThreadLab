...
export const action = async ({ request }: ActionFunctionArgs) => {
  ...
  let body: { sourceUrl?: string; rect?: { x?: number; y?: number; w?: number; h?: number } };
  ...
  if (!key) return json({ error: "Görsel bu mağazanın ürün görsellerinden olmalı" }, { status: 400 });
  ...
  if (![rect.x, rect.y, rect.w, rect.h].every(Number.isFinite) || rect.w <= 0 || rect.h <= 0) {
    return json({ error: "Alan geçersiz" }, { status: 400 });
  }
  ...
  try {
    ...
    if (!cut) return json({ error: "Alan çok küçük; biraz daha büyük çizin." }, { status: 400 });
    ...
    return json({ error: "Alan kesilemedi" }, { status: 500 });
  }
};
