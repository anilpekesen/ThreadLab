...
async function getGlobalStats() {
  const result = await query<{
    ...
    SELECT
      ...
      (SELECT COUNT(*)::text FROM ai_prompt_logs) as total_ai_logs
  `);
  ...
  return {
    ...
    totalAiLogs: parseInt(r?.total_ai_logs ?? "0", 10),
  };
}
...
export const loader = async ({ request }: LoaderFunctionArgs) => {
  if (!isAuthed(request)) {
    return json({
      ...
      aiLogs: [], aiLogsCount: 0, aiLogsPage: 0, filterShop: "",
    });
  }
  ...
  return json({ authed: true, tab: "shops", shops, globalStats, aiLogs: [], aiLogsCount: 0, aiLogsPage: 0, filterShop: "" });
};
...
export const action = async ({ request }: ActionFunctionArgs) => {
  ...
  if (intent === "logout") {
    return redirect("/panel", {
      headers: { "Set-Cookie": `${AUTH_COOKIE}=; Path=/panel; Max-Age=0; HttpOnly` },
    });
  }
  ...
  return redirect("/panel", {
    headers: {
      "Set-Cookie": `${AUTH_COOKIE}=${encodeURIComponent(secret)}; Path=/panel; HttpOnly; SameSite=Strict; Max-Age=${60 * 60 * 8}`,
    },
  });
};
...
const css = {
  ...
  statsGrid: { display: "grid", gridTemplateColumns: "repeat(5,1fr)", gap: 14, marginBottom: 22 } as React.CSSProperties,
  ...
  statValue: { fontSize: 26, fontWeight: 700, color: "#f1f5f9", marginTop: 4 },
  ...
  return (
    <div style={{ ...css.page, display: "flex", alignItems: "center", justifyContent: "center" }}>
      <div style={{ background: "#1e293b", border: "1px solid #334155", borderRadius: 14, padding: "40px 44px", width: 340 }}>
        ...
        <h1 style={{ margin: "0 0 24px", fontSize: 19, fontWeight: 700, color: "#f8fafc", textAlign: "center" }}>PrintLab Admin</h1>
        <Form method="post">
...
function StatCard({ label, value }: { label: string; value: number }) {
  return (
    <div style={css.statCard}>
      ...
      <div style={css.statValue}>{value.toLocaleString("tr-TR")}</div>
    </div>
  );
}
  ...
  return (
    <div style={css.card}>
      <table style={css.table}>
        <thead>
          <tr>
            {["Mağaza", "Plan", "Durum", "Top. Sipariş", "Bu Ay Sipariş", "AI/Ay", "BG/Ay", "Drive", "Son Sipariş"].map((h) => (
        ...
        <tbody>
          {shops.length === 0 && (
            <tr><td colSpan={9} style={{ ...css.td, textAlign: "center", color: "#475569", padding: 32 }}>Kayıt bulunamadı</td></tr>
          )}
          {shops.map((s) => (
            <tr key={s.shop} style={{ transition: "background .1s" }}
    ...
    <div>
      <div style={{ display: "flex", gap: 8, marginBottom: 14, alignItems: "center" }}>
        <Form method="get" style={{ display: "flex", gap: 8, alignItems: "center" }}>
          ...
          {filterShop && <a href="/panel?tab=ai-logs" style={{ fontSize: 13, color: "#94a3b8", textDecoration: "none" }}>Temizle ✕</a>}
        </Form>
      ...
      <div style={css.card}>
        <table style={css.table}>
          <thead>
            <tr>
              {["Tarih", "Mağaza", "Kullanıcı Promptu", "Optimize Edilmiş Prompt", "Sonuç"].map((h) => (
                <th key={h} style={css.th}>{h}</th>
              ))}
            </tr>
          </thead>
      ...
      {totalPages > 1 && (
        <div style={{ display: "flex", gap: 8, marginTop: 16, justifyContent: "center", alignItems: "center" }}>
          {page > 0 && (
            <a href={`/panel?tab=ai-logs&page=${page - 1}${filterShop ? `&shop=${encodeURIComponent(filterShop)}` : ""}`}
  ...
  return (
    <div style={css.page}>
      <header style={css.header}>
        <span style={css.logo}>🖨 PrintLab Admin</span>
        <nav style={{ display: "flex", gap: 4, flex: 1 }}>
          <a href="/panel?tab=shops" style={css.navLink(data.tab === "shops")}>Mağazalar</a>
          <a href="/panel?tab=ai-logs" style={css.navLink(data.tab === "ai-logs")}>AI Logları</a>
        </nav>
        <Form method="post">
      ...
      <main style={css.main}>
        {data.globalStats && (
          <div style={css.statsGrid}>
            <StatCard label="Toplam Mağaza" value={data.globalStats.totalShops} />
            <StatCard label="Toplam Sipariş" value={data.globalStats.totalOrders} />
            <StatCard label="Toplam AI Üretim" value={data.globalStats.totalAiGens} />
            <StatCard label="Toplam BG Kaldırma" value={data.globalStats.totalBgRemovals} />
            <StatCard label="AI Log Kaydı" value={data.globalStats.totalAiLogs} />
          </div>
        )}
