...
export default function SavedPanel({ onLoad }: Props) {
  const { savedDesigns, removeSavedDesign, config } = useDesignerStore();
  ...
  if (savedDesigns.length === 0) {
    return (
      <div className="flex min-h-[240px] flex-col items-center justify-center rounded-[28px] border border-dashed border-slate-200 bg-slate-50/80 px-6 text-center text-slate-400">
        ...
        <p className="text-sm font-semibold text-slate-500">{t.savedEmptyTitle}</p>
        <p className="mt-1 text-xs font-medium text-slate-400">{t.savedEmptyHint}</p>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-5">
      <div>
        <p className="text-[11px] font-black uppercase tracking-[0.22em] text-blue-600">{t.savedArchive}</p>
        <p className="text-sm font-semibold text-slate-500">{savedDesigns.length} {t.savedCount}</p>
        <p className="mt-1 flex items-center gap-1 text-xs font-medium text-slate-400">
          <CloudUpload className="h-3.5 w-3.5" />
          {customerLoggedIn ? t.savedStorageAccount : t.savedStorageLocal}
        </p>
      </div>
